...
class bnb_fp4_Compressor(Compressor):
    ...
    def compress(self, x: torch.Tensor) -> Payload:
        x_lp, state = bnb.functional.quantize_4bit(x, quant_type="nf4")  # packed uint8 + state
...
for k in [10000, 20000, 50000, 100000, 0]:
    run_ppl_eval(
        wandb_run_name = f"bnb_nf4_Compressor_{k}",
        dtype = "bf16",
        load_in_8bit = True,
        compressor=bnb_fp4_Compressor(),
        first_k_tokens=k,
        result_dir = str(REPO_ROOT / "results" / "compressor_bnb_nf4"),
    )
    gc.collect()
